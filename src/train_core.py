...
def train_pseudo_contravarsal(configs, model, train_loader, optimizer, criterion_CE, criterion_KL):
    iou_obj = iou.IoU(n_classes=configs.N_classes)
    logs = log()
    model.train()
    train_data_classes = torch.zeros((1, configs.N_classes), requires_grad=True).to(configs.device)
    pseudo_data_classes = torch.zeros((1, configs.N_classes), requires_grad=True).to(configs.device)
    for step, ((image, label, name), (image_ps_pos, _, name_ps_pos), (image_ps_neg, _, name_ps_neg)) in enumerate(train_loader):
        image_shape = image.shape
        ...
        # ---- train pseudo ------
        image_ps_pos = image_ps_pos.to(configs.device)
        outputs_ps = model(image_ps_pos)
        label_ps = outputs_ps.clone().detach()
        label_ps_shape = label_ps.shape
        percent = torch.quantile(torch.nn.Softmax(dim=1)(label_ps).view(label_ps_shape[0], -1), torch.tensor(0.7, device=configs.device), dim=1)
        percent = percent.repeat(torch.prod(torch.tensor(label_ps_shape[1:])), 1).permute(1, 0).reshape(label_ps_shape)
        ...
        # save predict images when last epoch
        if configs.now_epoch == configs.Segmentation_Epoch:
            image_ps_pos = image_ps_pos.to('cpu').numpy()
            label_ps = label_ps.to('cpu').numpy()
            image_overlap.save_overlap_image(configs, image_ps_pos, label_ps, name_ps_pos)
        ...
        # ---- controversial ----
        image_ps_neg = image_ps_neg.to(configs.device)
        outputs_ps_neg = model(image_ps_neg)
        outputs_ps_neg = torch.nn.Softmax(dim=1)(outputs_ps_neg)
        outputs_ps_neg_area = outputs_ps_neg.sum(dim=(2, 3)) / (image_shape[2] * image_shape[3])
        loss_C_KL_neg = torch.mean(criterion_KL(outputs_ps_neg_area, outputs_area))
        logs.train_loss_CKL_neg.append(loss_C_KL_neg.item())
        # -----------------------


        loss = loss_CE + loss_ps*0.1 + loss_C_KL*0.1 + loss_C_KL_neg*0.1
        logs.train_loss.append(loss.item())
